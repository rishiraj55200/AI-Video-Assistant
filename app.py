...
def safe_import(module_name: str, attribute: str):
    try:
...
process_input, audio_error = safe_import("utils.audio_processor", "process_input")
transcribe_all, transcribe_error = safe_import("core.transcriber", "transcribe_all")
summarize, summarize_error = safe_import("core.summarizer", "summarize")
generate_title, title_error = safe_import("core.summarizer", "generate_title")
extract_action_items, extract_error = safe_import("core.extractor", "extract_action_items")
extract_key_decisions, decisions_error = safe_import("core.extractor", "extract_key_decisions")
extract_questions, questions_error = safe_import("core.extractor", "extract_questions")
build_rag_chain, rag_error = safe_import("core.rag_engine", "build_rag_chain")
ask_question, ask_error = safe_import("core.rag_engine", "ask_question")

IMPORT_ERRORS = {
    name: error
    for name, error in [
        ("utils.audio_processor", audio_error),
        ("core.transcriber", transcribe_error),
        ("core.summarizer", summarize_error),
        ("core.extractor", extract_error),
        ("core.rag_engine", rag_error),
    ]
...
if IMPORT_ERRORS:
    st.error("The app could not start because one or more required modules failed to import.")
    for module_name, error in IMPORT_ERRORS.items():
        st.code(f"{module_name}: {error}")
    st.info("Install the missing packages in the same Python environment that runs Streamlit, then refresh the page.")
    st.stop()
